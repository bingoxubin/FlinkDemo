...
public class KafkaTwo {
	public static void main(String[] args) {
		...
		// 消费 kafka 数据
		new KafkaSource<String>(ctx).topic("bingo").getDataStream().map(record -> record.value()).print("bingo");
		new KafkaSource<String>(ctx).topic("bingo1").getDataStream().map(record -> record.value()).print("bingo1");
		// 启动任务
		ctx.start();
	}
